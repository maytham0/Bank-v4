//اخذته من قناة الاستاذ لان مرتب اكثر من لي مشتغلة اني 
...
public:
	...
	clsDate(short Day, short Month, short Year,short Hour,short Minut,short Second)
	{
	...
	static string DateToStringWith_HMS(clsDate Date)
	{
		return  to_string(Date.Day) + "/" + to_string(Date.Month) + "/" + to_string(Date.Year)+"-" + to_string(Date._Hour) +":"+to_string(Date._Min)+":"+to_string (Date._Second);
	}
	...
	static clsDate SystemDateWithPerSec()
	{
